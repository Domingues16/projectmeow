	...
	run: async (client, message, args) => {
		...
		if (!membro1 || !membro2)
			return message.channel.send(
				'Lembre-se de mencionar dois usuários para shippar.'
			);
		if (membro1 === membro2)
		...
		if (amor > 60) {
			...
			emoticon =
				'https://cdn.glitch.com/00963c7e-8e86-4a55-8d85-36add9e330d7%2Femoji_3-1.png?v=1593652255529'; //imagem de sei lá
		} else {
			emoticon =
		...
		if (amor > 90) {
			desc =
				':sparkling_heart: Hm..., vai rolar ou não vai? :sparkling_heart:\n' +
				membro1.user.username +
				'``\n``' +
				membro2.user.username +
				'``\n:heart: ``' +
				nomeship +
				'`` Esse é o casal perfeito! :heart:';
		} else if (amor >= 70) {
			desc =
				':sparkling_heart: Hm.. vai rolar ou não vai? :sparkling_heart:\n``' +
				membro1.user.username +
				...
				'``\n:neutral_face: ``' +
				nomeship +
				'`` Esses aqui já tão se pegando e não contaram para ninguém! :neutral_face:';
		} else if (amor >= 45) {
			desc =
				':sparkling_heart: Hm... Vai rolar ou não vai? :sparkling_heart:\n``' +
				membro1.user.username +
